...
public class Request {
    ...
    public async Task sendUpdateUserAsync(string newUsername, string newPassword, string newIsAdmin) {
        ...
        if (token != null) {
            ...
            StringContent stringContent = new(json);
            var test = await client.PutAsync(update_user_url, stringContent);
        }
    }
    ...
    public async Task sendChangeUserPasswordAsync(string username, string newPassword, string oldPassword){
        HttpClient client = new();
        string token = await getTokenJsonAsync(socket);
        if(token != null){
            string change_password_url = $"http://{socket}/api/user/{await getIdJsonAsync(socket, token, username)}";
            client.DefaultRequestHeaders.Add("x-nd-authorization", "Bearer " + token);
            json_data json_Data = new(){
                changePassword = true,
                userName = username,
            ...
            StringContent stringContent = new(json);
            await client.PutAsync(change_password_url, stringContent);
        }
    }
